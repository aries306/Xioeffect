...
const XIO_CHAT = (() => {
  ...
  async function ensureWorkspace() {
    ...
    const syncResponse = await fetch("/api/workspace", {
      ...
      body: JSON.stringify({ workspaceId, context: {
        ...
        goals: E().state.goals, projects: E().state.projects,
      } }),
    });
  ...
  function typing(show) {
    const { thread } = els(); let t = thread.querySelector(".typing-wrap");
    if (show && !t) { t = document.createElement("div"); t.className = "chat-msg xio typing-wrap"; t.innerHTML = `<div class="chat-ava xio">✦</div><div class="chat-bubble typing"><span></span><span></span><span></span></div>`; thread.appendChild(t); }
    else if (!show && t) t.remove();
    thread.scrollTop = thread.scrollHeight;
  }

  function renderSuggestions() {
    const { suggest } = els(); const g = E().topGoal();
  ...
  function restore() {
    const { thread } = els(); thread.innerHTML = ""; const hist = E().state.chat.slice(-40);
    if (!hist.length) { greet(); return; }
    hist.forEach((m) => bubble(m.role === "user" ? "user" : "xio", m.text)); greetedOnce = true;
  }

  function greet() {
    if (greetedOnce) return; greetedOnce = true; const name = E().state.profile.name;
    const greetText = name ? `${E().greeting()}\n\nYour workspace is connected to Astara and the Memory Fabric. What's on your mind?` : `Hey — I'm Astara inside XIO. Your workspace context and approved memory can inform our conversations. What's on your mind?`;
    typing(true); setTimeout(() => { typing(false); bubble("xio", greetText); E().state.chat.push({ role: "xio", text: greetText, at: Date.now() }); E().save(); }, 500);
  }
  ...
  async function submit() {
    ...
    try {
      ...
      if (!response.ok) throw new Error(data.error || "Astara could not process the request");
      conversationId = data.conversationId || conversationId; typing(false);
      bubble("xio", data.answer, { proposals: data.memoryProposals || [], recommendation: data.recommendation });
  ...
  async function mount() { try { await ensureWorkspace(); } catch { /* the protected API will report the server error on submit */ } restore(); renderSuggestions(); setTimeout(() => els().field.focus(), 60); }
