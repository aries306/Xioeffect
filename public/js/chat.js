...
const XIO_CHAT = (() => {
  ...
  function bubble(role, text, options = {}) {
    ...
    if (options.proposals?.length) {
      options.proposals.forEach((proposal) => {
  ...
  function greet() {
    ...
    typing(true);
    setTimeout(() => { typing(false); bubble("xio", greetText); E().state.chat.push({ role: "xio", text: greetText, at: Date.now() }); E().save(); }, 500);
  }
  ...
  async function mount() { try { await ensureWorkspace(); } catch { /* protected route will surface auth/server errors through chat */ } restore(); renderSuggestions(); setTimeout(() => els().field.focus(), 60); }
