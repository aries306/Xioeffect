...
export async function POST(request: Request) {
  ...
  const sql = db();
  const rows = await sql`select id, token_ciphertext, token_iv, token_auth_tag, refresh_token_ciphertext, refresh_token_iv, refresh_token_auth_tag, access_token_expires_at, refresh_token_expires_at from github_connections where user_id=${user.id} limit 1`;
  if (!rows[0]) return Response.json({ error: "GitHub is not connected" }, { status: 404 });
  const token = await usableAccessToken(user.id, rows[0] as Connection, sql);
  ...
  for (const repo of repos) {
    ...
    try {
      ...
      const existing = await sql`select last_synced_commit from github_repositories where id=${repositoryId}`;
      ...
      for (const item of candidates) {
        const known = await sql`select blob_sha from github_documents where repository_id=${repositoryId} and path=${item.path}`;
        ...
        const researchRows = await sql`insert into research_records (user_id, source_type, source_id, title, summary, content, provenance) values (${user.id}, 'github', ${changed[0].id}, ${title}, ${excerpt.slice(0, 500)}, ${excerpt}, ${JSON.stringify(provenance)}::jsonb) returning id`;
        if (researchRows[0]) {
          research++;
