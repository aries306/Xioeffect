...
async function usableAccessToken(userId: string, connection: Connection, sql: ReturnType<typeof db>) {
  ...
  await sql`
    update github_connections set
      token_ciphertext=${encrypted.ciphertext}, token_iv=${encrypted.iv}, token_auth_tag=${encrypted.authTag},
      refresh_token_ciphertext=${refresh?.ciphertext ?? null}, refresh_token_iv=${refresh?.iv ?? null}, refresh_token_auth_tag=${refresh?.authTag ?? null},
      access_token_expires_at=${refreshed.accessTokenExpiresAt ?? null}, refresh_token_expires_at=${refreshed.refreshTokenExpiresAt ?? null},
      scopes=case when ${refreshed.scopes} <> '' then ${refreshed.scopes} else scopes end, updated_at=now()
    where id=${connection.id} and user_id=${userId}
  `;
...
export async function POST(request: Request) {
  ...
  const rows = await sql<Connection[]>`
    select id, token_ciphertext, token_iv, token_auth_tag, refresh_token_ciphertext, refresh_token_iv, refresh_token_auth_tag,
  ...
  for (const repo of repos) {
    ...
    const shouldSync = requestedRepoIds ? requestedRepoIds.includes(repositoryId) : true;
    ...
    try {
      const headSha = await fetchBranchHeadSha(token, repo.full_name.split("/")[0], repo.name, repo.default_branch);
      ...
      for (const item of candidates) {
        ...
        const researchRows = await sql<{ id: string }[]>`
          ...
          values (${user.id}, 'github', ${changed[0].id}, ${title}, ${excerpt.slice(0, 500)}, ${excerpt}, ${JSON.stringify(provenance)}::jsonb)
          returning id
        `;
        if (researchRows[0]) {
          research++;
          ...
          await sql`
            insert into nexus_ingest_events (user_id, research_id, event_type, payload, dedupe_key)
            values (${user.id}, ${researchRows[0].id}, 'research.github.document', ${JSON.stringify({ title, repository: repo.full_name, path: item.path, commit_sha: headSha, blob_sha: item.sha, content_hash: contentHash })}::jsonb, ${dedupeKey})
