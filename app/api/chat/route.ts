...
export async function POST(request: Request) {
  try {
    ...
    const preferences = await sql`
      select learning_enabled, ask_before_memory from preferences where user_id=${userId} limit 1
    `;
    const learningEnabled = preferences[0]?.learning_enabled ?? true;
    const askBeforeMemory = preferences[0]?.ask_before_memory ?? true;

    const result = await runAstara({
    ...
    if (learningEnabled && !askBeforeMemory) {
      for (const candidate of candidates) {
        persisted.push(await createMemory({
          ...
          scope: { conversationId: result.conversationId },
          confirmed: false,
        }));
      }
    }
    ...
    console.info(JSON.stringify({
      event: "astara.chat.completed",
      userId,
      workspaceId: workspace.id,
      conversationId: result.conversationId,
      memoryCount: result.evidence.length,
    ...
    return Response.json({
      ...result,
      memoryProposals: askBeforeMemory ? candidates : [],
      persistedMemoryIds: persisted.map((memory) => memory.id),
    });
  } catch (error) {
