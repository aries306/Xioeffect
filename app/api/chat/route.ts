...
export async function POST(request: Request) {
  try {
    ...
    const persisted = [];
    if (learningEnabled && !askBeforeMemory) {
      for (const candidate of candidates) {
        persisted.push(await createMemory({
    ...
    console.info(JSON.stringify({
      event: "astara.chat.completed", userId, workspaceId: workspace.id, conversationId: result.conversationId,
      memoryCount: result.evidence.length, candidateCount: candidates.length, persistedMemoryCount: persisted.length,
      durationMs: result.diagnostics.durationMs,
    }));
    return Response.json({ ...result, memoryProposals: askBeforeMemory ? candidates : [], persistedMemoryIds: persisted.map((memory) => memory.id) });
  } catch (error) {
