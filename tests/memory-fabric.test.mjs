...
test('Workspace and memory routes enforce authenticated service boundaries', async () => {
  ...
  assert.match(memoryRoute, /getAuthorizedWorkspace/);
  assert.match(chatRoute, /runAstara/);
});
...
test('Live authenticated loop exercises the real server when test credentials are supplied', async (t) => {
  ...
  if (chatBody.memoryProposals?.length) {
    ...
    const feedback = await fetch(`${baseUrl}/api/feedback`, { method: 'POST', headers, body: JSON.stringify({ workspaceId: workspaceBody.workspace.id, memoryId: memoryBody.memory.id, signal: 'useful', note: 'Integration test feedback', recommendationOutcomeId: undefined }) });
