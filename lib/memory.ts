...
function rowToMemory(row: Record<string, unknown>): MemoryRecord {
  return {
    ...
    confidence: Number(row.confidence), relevance: Number(row.relevance),
    lifecycleState: String(row.lifecycle_state) as MemoryRecord["lifecycleState"],
    confirmed: Boolean(row.confirmed), scope: (row.scope ?? {}) as MemoryScope,
    provenance: (row.provenance ?? {}) as Record<string, unknown>, source: String(row.source),
...
export async function retrieveContextualMemories(workspaceId: string, query: string, context: Record<string, unknown> = {}, limit = 8) {
  ...
  const rows = await sql`
    ...
    from memories where workspace_id=${workspace.id} and lifecycle_state in ('active','dormant') and active=true and confidence >= 5
  ...
  const scored = rows.map((row) => {
    ...
    return { row, score: (lexical + contextual + confidence * 2 + relevance * 2 + recency) * dormantPenalty };
  ...
  await Promise.all(scored.map(async ({ row, score }) => {
    ...
    await sql`
      ...
      values (${row.id}, ${workspace.id}, (select owner_user_id from workspaces where id=${workspace.id}), 'retrieved', ${row.confidence}, ${row.confidence}, ${row.relevance}, ${row.relevance}, ${row.lifecycle_state}, ${row.lifecycle_state}, 'contextual-retrieval', ${JSON.stringify({ score: Number(score.toFixed(3)) })}::jsonb)
    `;
  }));
...
export async function createMemory(input: {
  workspaceId: string; text: string; category?: string; confidence?: number; relevance?: number; source?: string;
  scope?: MemoryScope; provenance?: Record<string, unknown>; confirmed?: boolean;
}) {
  ...
  if (existing[0]) {
    const before = Number(existing[0].confidence), relevanceBefore = Number(existing[0].relevance);
    const after = Math.min(99, before + 8), relevanceAfter = Math.min(100, relevanceBefore + 4);
...
export async function applyMemoryFeedback(input: {
  workspaceId: string; memoryId: string;
  signal: "confirm" | "contradict" | "useful" | "not_useful" | "reactivate" | "supersede" | "dismiss";
  note?: string; recommendationOutcomeId?: string;
}) {
  const { userId, workspace } = await getAuthorizedWorkspace(input.workspaceId);
  const sql = db();
  const rows = await sql`select * from memories where id=${input.memoryId} and workspace_id=${workspace.id} and user_id=${userId} limit 1`;
  ...
  if (input.signal === "confirm" || input.signal === "useful") {
    confidence = Math.min(99, confidence + 12); relevance = Math.min(100, relevance + 8); lifecycle = "active";
    eventType = input.signal === "confirm" ? "confirmed" : "feedback";
  } else if (input.signal === "contradict") {
    confidence = Math.max(5, confidence - 25); relevance = Math.max(0, relevance - 10); lifecycle = confidence <= 25 ? "dormant" : "active";
  ...
  const updated = await sql`
    update memories
    set confidence=${confidence}, relevance=${relevance}, lifecycle_state=${lifecycle}, active=${lifecycle === 'active' || lifecycle === 'dormant'}, updated_at=now(),
        last_confirmed_at=case when ${input.signal === 'confirm' || input.signal === 'useful'} then now() else last_confirmed_at end
