...
function encryptionKey() {
  const raw = Buffer.from(required("GITHUB_TOKEN_ENCRYPTION_KEY"), "base64");
  if (raw.length !== 32) throw new Error("GITHUB_TOKEN_ENCRYPTION_KEY must be a base64-encoded 32-byte key");
  return raw;
}
...
export function githubAuthorizationUrl(state: string, codeChallenge: string) {
  const params = new URLSearchParams({ client_id: required("GITHUB_CLIENT_ID"), redirect_uri: required("GITHUB_REDIRECT_URI"), scope: "repo read:user user:email offline_access", state, code_challenge: codeChallenge, code_challenge_method: "S256" });
...
export type GitHubTokenSet = { accessToken: string; refreshToken?: string; accessTokenExpiresAt?: Date; refreshTokenExpiresAt?: Date; scopes: string };

function tokenSet(body: Record<string, unknown>, fallbackRefreshToken?: string): GitHubTokenSet {
  const now = Date.now();
  return {
    accessToken: body.access_token as string,
    refreshToken: typeof body.refresh_token === "string" ? body.refresh_token : fallbackRefreshToken,
    accessTokenExpiresAt: typeof body.expires_in === "number" ? new Date(now + body.expires_in * 1000) : undefined,
    refreshTokenExpiresAt: typeof body.refresh_token_expires_in === "number" ? new Date(now + body.refresh_token_expires_in * 1000) : undefined,
    scopes: typeof body.scope === "string" ? body.scope : "",
  };
}

export async function exchangeGitHubCode(code: string, codeVerifier: string): Promise<GitHubTokenSet> {
  ...
  return tokenSet(body as Record<string, unknown>);
}

export async function refreshGitHubToken(refreshToken: string): Promise<GitHubTokenSet> {
  const response = await fetch(GITHUB_TOKEN, { method: "POST", headers: { Accept: "application/json" }, body: new URLSearchParams({ client_id: required("GITHUB_CLIENT_ID"), client_secret: required("GITHUB_CLIENT_SECRET"), grant_type: "refresh_token", refresh_token: refreshToken }), cache: "no-store" });
  const body = await response.json();
  if (!response.ok || body.error || !body.access_token) throw new Error(body.error_description ?? "GitHub token refresh failed");
  return tokenSet(body as Record<string, unknown>, refreshToken);
}
...
export async function fetchRepositoryTree(token: string, owner: string, repo: string, ref: string) { return githubApi<{ sha?: string; tree: Array<{ path: string; mode: string; type: string; sha: string; size?: number; url: string }>; truncated: boolean }>(token, `/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/git/trees/${encodeURIComponent(ref)}?recursive=1`); }
export async function fetchBranchHeadSha(token: string, owner: string, repo: string, branch: string) { const ref = await githubApi<{ object: { sha: string } }>(token, `/repos/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/git/ref/heads/${encodeURIComponent(branch)}`); return ref.object.sha; }
