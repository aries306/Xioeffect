...
select u.id from users u where not exists (select 1 from workspaces w where w.owner_user_id = u.id);
insert into workspace_members (workspace_id, user_id, role)
select w.id, w.owner_user_id, 'owner' from workspaces w
where not exists (select 1 from workspace_members wm where wm.workspace_id = w.id and wm.user_id = w.owner_user_id);

alter table goals add column if not exists workspace_id uuid;
update goals g set workspace_id=w.id from workspaces w where g.workspace_id is null and w.owner_user_id=g.user_id;
create index if not exists goals_workspace_idx on goals(workspace_id, status, updated_at desc);
alter table goals add constraint goals_workspace_fk foreign key (workspace_id) references workspaces(id) on delete cascade;
...
update memories m set workspace_id=w.id from workspaces w where m.workspace_id is null and w.owner_user_id=m.user_id;
...
update conversations c set workspace_id=w.id from workspaces w where c.workspace_id is null and w.owner_user_id=c.user_id;
...
create table if not exists memory_events (
  id uuid primary key default gen_random_uuid(), memory_id uuid not null references memories(id) on delete cascade,
  ...
  confidence_before smallint, confidence_after smallint, relevance_before smallint, relevance_after smallint,
  lifecycle_before text, lifecycle_after text, source text, metadata jsonb not null default '{}', created_at timestamptz not null default now()
);
...
create table if not exists recommendation_outcomes (
  ...
  user_id uuid not null references users(id) on delete cascade, conversation_id uuid references conversations(id) on delete set null,
  recommendation text not null, outcome text not null check (outcome in ('accepted','rejected','partial','unknown')),
  feedback text, metadata jsonb not null default '{}', created_at timestamptz not null default now()
);
